...
% Choose reator type:
reactorType = 'batch'; % 'batch' or 'column'

% Choose model type:
param.modelType = 'ann'; %'lp' or 'ann'
...
switch reactorType
    case 'batch'
        ...
        if strcmp(param.modelType,'lp')
            % tspan = [0 35];
            tspan = [0 27];
        elseif strcmp(param.modelType,'ann')
            tspan = [0 35];
            % tspan = [0 27];
        else
            error('check your modelType')
        end
...
if exitflag <= 0
    exitflag
    warning('incorrect solution in maximizing biomass: goLac')
    bioMax = 0;
else
    bioMax = x(idx.rxnBio);
end
...
if exitflag <= 0
    exitflag
    warning('incorrect solution in maximizing pyruvate: goLac')
    pyrMax = 0;
else
    pyrMax = x(idx.rxnPyr);
end
...
if exitflag <= 0
    exitflag
    warning('incorrect solution in maximizing acetate: goLac')
    x = zeros(length(x),1);
end
...
if exitflag <= 0
    exitflag
    warning('incorrect solution in maximizing biomass: goPyr')
else
    bioMax = x(idx.rxnBio);
end
...
if exitflag <= 0
    exitflag
    warning('incorrect solution in maximizing acetate: goPyr')
    x = zeros(length(x),1);
end
x_idx = [x(idx.rxnLac) x(idx.rxnPyr) x(idx.rxnAce) x(idx.rxnOxy) x(idx.rxnBio)];
...
if exitflag <= 0
    exitflag
    warning('incorrect solution in maximizing biomass: goAce')
    x = zeros(length(x),1);
end
x_idx = [x(idx.rxnLac) x(idx.rxnPyr) x(idx.rxnAce) x(idx.rxnOxy) x(idx.rxnBio)];